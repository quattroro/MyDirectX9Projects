...
### Actor를 상속받는 플루프린트 클래스에 생성한 변수의 디테일 패널 파라미터 설명

* 인스턴스 편집가능 -> 해당 파라미터를 사용하면 액터를 레벨에 배치했을 때 이 벡터 변수를 외부에서 보이도록 설정할 수 있으므로, 이 액터의 각 인스턴스에서 이 변수를 편집할 수 있다.

* 3D 웨젯 표시 -> 해당 파라미터를 사용하면 뷰포트에서 시각적으로 표시된 3D 트랜스폼 위젯을 사용해 벡터 값의 위치를 설정할 수 있다. 또한 3D 위젯 표시 옵션은 vector와 transform등 액터 트랜스폼과 관련된 변수에만 사용 가능하다.